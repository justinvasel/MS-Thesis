...
\chapter{Neutrino Production in Supernovae}
	...
	\section{Shell Burning}
		...
		% This limiting mass is the upper bound for objects to counteract their own gravitational pressure with electron degeneracy pressure. When nuclear synthesis ceases in the core, the radiation pressure that has supported the rest of the star disappears, and the massive core collapses. This collapse will be discussed in detail in the following section, but, first I will outline the final steps of the shell burning regime.

		% The core mass of a sufficiently massive star will be driven towards $M_\text{Ch}$ during the silicon burning stage. Silicon burning occurs at temperatures that require $M_\text{core}\gtrsim M_\text{Ch}$. If the mass of the core is not sufficiently large, the core will contract like every stage before until $M_\text{core}\approx M_\text{Ch}$. If instead the core mass is larger than $M_\text{Ch}$ at any point before nuclear fusion has permanently ceased, the rate of neutrino cooling is relatively high,\footnote{The rate at which neutrino cooling occurs varies as a high power of $T$\cite{astrophysics}.} causing the core to shrink and its mass to approach $M_\text{Ch}$.
